...
## 4. 公開 URL とゲーム内の「トークン API URL」

デプロイ後、プロジェクトに付く URL は次のような形です。

```text
https://<プロジェクト名>.pages.dev/
```

SkyWay 用トークン API の **フル URL** は次です。

```text
https://<プロジェクト名>.pages.dev/api/skyway-token
```

ゲームの **オンライン対戦 → インターネット（SkyWay）** を選び、表示される **トークン API URL** に、上記の `https://.../api/skyway-token` をそのまま貼り付けてください。  
（未入力の場合は「今開いているサイト」と同じオリジンの `api/skyway-token` を試すため、Pages 上では通常このままで動きます。別ドメインにしたいときだけフル URL を指定します。）
